...
module keccak_TI3_PIPE(
    ...
    wire kr_done; // keccak round done
    wire ss_1, ss_2, ss_3;
    
    reg [1:0] shuff_op;
    always @(posedge clk) begin
        if(load)                        shuff_op <= random_data_1[1:0] ^ random_data_2[1:0];
        else if(start_clk & kr_done)    shuff_op <= shuff_op ^ st_i_w_1[1:0];
        else                            shuff_op <= shuff_op;
    end
    
    wire [1:0] rand_op; 
    assign rand_op = (shuff_op[1] + shuff_op[0]);
    
    kecti3_round_pipe keccak (
        .clk(clk),
        .rst(rst & start_clk),
        .rand_op(rand_op),
        .flag_done(kr_done),
    ...
    */
    ...
    always @(posedge clk) begin
        if(!rst) begin
            ...
            st_i_w_3 <= {random_data_2[799:0] ^ random_data_1[799:0], random_data_2[1599:800] ^ random_data_1[1599:800]};
            */
        end  
        else begin
            if(load & !load_clk & !flag_masked_prf) begin
                st_i_w_1 <= random_data_1;
                st_i_w_2 <= random_data_2;
                st_i_w_3 <= st_i_w_3;
            end  
            else if(load & !load_clk & flag_masked_prf) begin
                st_i_w_1 <= input_data_1;
                st_i_w_2 <= input_data_2;
                st_i_w_3 <= st_i_w_3;
            end 
            else if(load_clk & !flag_masked_prf) begin
                st_i_w_1 <= st_i_w_1;
                st_i_w_2 <= st_i_w_2;
                st_i_w_3 <= st_i_w_1 ^ st_i_w_2 ^ input_data;
            end  
            else if(load_clk & flag_masked_prf) begin
                st_i_w_1 <= st_i_w_1;
                st_i_w_2 <= st_i_w_2;
            ...
            else if(start_clk & !end_op) begin 
                if(ss_1)    st_i_w_1 <= st_o_w_1;
                else        st_i_w_1 <= st_i_w_1;
                
                if(ss_2)    st_i_w_2 <= st_o_w_2;
                else        st_i_w_2 <= st_i_w_2;
                
                if(ss_3)    st_i_w_3 <= st_o_w_3;
                else        st_i_w_3 <= st_i_w_3;
            end
            else begin 
                st_i_w_1 <= st_i_w_1;
                st_i_w_2 <= st_i_w_2;
    ...
    */
    ...
    always @(posedge clk) begin
        if(!rst) rndc_r <= 0;
            ...
            if(load | read)                         rndc_r <= 8'h01;
            else if(start_clk & !end_op & kr_done)  rndc_r <= rc_o_w;
            else                                    rndc_r <= rndc_r;
        end
    end
    ...
    always @(posedge clk) begin
        if(!rst)                                    round_counter <= 0;
            ...
            if(load | read)                         round_counter <= 0;
            else if(start_clk & !end_op & kr_done)  round_counter <= round_counter + 1;
            else                                    round_counter <= round_counter;
        end
    end
    ...
    always @(posedge clk) begin
        if(!rst)                                    end_op_reg <= 0;
            ...
            else if(round_counter == 23 & kr_done)  end_op_reg <= 1;
    ...
    */
    ...
    always @(posedge clk) begin
        if(!rst) rndc_r <= 0;
            ...
            if(load | read)                         rndc_r <= 8'h01;
            else if(start_clk & !end_op)            rndc_r <= rc_o_w;
            else                                    rndc_r <= rndc_r;
        end
    end
    ...
    always @(posedge clk) begin
        if(!rst)                                    round_counter <= 0;
            ...
            if(load | read)                         round_counter <= 0;
            else if(start_clk & !end_op)            round_counter <= round_counter + 1;
            else                                    round_counter <= round_counter;
        end
    end
...
module kecti3_round_pipe (
    input wire clk,
    input wire rst,
    input wire [1:0] rand_op,
    output reg [1599:0] a_o,           //  shares out
    output reg [1599:0] b_o,
    output reg [1599:0] c_o,
    input wire  [1599:0] a_i,           //  shares in
    input wire  [1599:0] b_i,
    input wire  [1599:0] c_i,
    output wire [7:0]    r_o,           //  rc out
    ...
    reg [1:0] sel_share;
    reg [1599:0] la_w, lb_w, lc_w;
    
    assign flag_done    = (sel_share == 2'b11) ? 1'b1 : 1'b0;
    
    assign ss_1         =   (sel_share == 2'b10 & rand_op == 2'b00) ? 1'b1 :
                            (sel_share == 2'b11 & rand_op == 2'b01) ? 1'b1 :
                            (sel_share == 2'b11 & rand_op == 2'b10) ? 1'b1 : 1'b0;
    assign ss_2         =   (sel_share == 2'b10 & rand_op == 2'b01) ? 1'b1 :
                            (sel_share == 2'b11 & rand_op == 2'b10) ? 1'b1 :
                            (sel_share == 2'b11 & rand_op == 2'b00) ? 1'b1 : 1'b0;
    assign ss_3         =   (sel_share == 2'b10 & rand_op == 2'b10) ? 1'b1 :
                            (sel_share == 2'b11 & rand_op == 2'b00) ? 1'b1 :
                            (sel_share == 2'b11 & rand_op == 2'b01) ? 1'b1 : 1'b0;
    ...
    */
    ...
    kecti3_lin  lin_c   (   .l_o(lc_w), .x_i(c_i) );
    */
    wire    [1599:0]    lin_i_0, lin_i_1;
    wire    [1599:0]    lin_o_0, lin_o_1;
    /*
    assign lin_i =  (sel_share == 3'b000) ? a_i :
                    (sel_share == 3'b001) ? b_i :
                    (sel_share == 3'b010) ? c_i : (a_i ^ b_i ^ c_i);
    */
    
    assign lin_i_0 =    (sel_share == 2'b00 & rand_op == 2'b00) ? a_i :
                        (sel_share == 2'b00 & rand_op == 2'b01) ? b_i :
                        (sel_share == 2'b00 & rand_op == 2'b10) ? c_i :
                        (sel_share == 2'b01 & rand_op == 2'b00) ? c_i :
                        (sel_share == 2'b01 & rand_op == 2'b01) ? a_i :
                        (sel_share == 2'b01 & rand_op == 2'b10) ? b_i :
                        (a_i ^ b_i); 
                        
    assign lin_i_1 =    (sel_share == 2'b00 & rand_op == 2'b00) ? b_i :
                        (sel_share == 2'b00 & rand_op == 2'b01) ? c_i :
                        (sel_share == 2'b00 & rand_op == 2'b10) ? a_i :
                        (b_i ^ c_i); 

    kecti3_lin  kecti3_lin_0   (   .l_o(lin_o_0), .x_i(lin_i_0) );
    kecti3_lin  kecti3_lin_1   (   .l_o(lin_o_1), .x_i(lin_i_1) );
    ...
    generate
        for (i = 0; i < 1600; i = i + 320) begin
            kecti3_tchi tchi_a  (   .f_o(  a_w[319 + i: i]  ),
                                    .x_i( la_w[319 + i: i]  ),
                                    .y_i( lb_w[319 + i: i]  )   );
            kecti3_tchi tchi_b  (   .f_o(  b_t[319 + i: i]  ),
                                    .x_i( lb_w[319 + i: i]  ),
                                    .y_i( lc_w[319 + i: i]  )   );
            kecti3_tchi tchi_c  (   .f_o(  c_t[319 + i: i]  ),
                                    .x_i( lc_w[319 + i: i]  ),
                                    .y_i( la_w[319 + i: i]  )   );
        end
    endgenerate
    */
    ...
    wire    [1599:0]    x_i_0, x_i_1;            //  share without rc
    wire    [1599:0]    y_i_0, y_i_1;            
    wire    [1599:0]    f_o_0, f_o_1;    
  
                    
    assign x_i_0 =      (sel_share == 2'b01 & rand_op == 2'b00) ? la_w :
                        (sel_share == 2'b01 & rand_op == 2'b01) ? lb_w :
                        (sel_share == 2'b01 & rand_op == 2'b10) ? lc_w :
                        (sel_share == 2'b10 & rand_op == 2'b00) ? lc_w :
                        (sel_share == 2'b10 & rand_op == 2'b01) ? la_w :
                        (sel_share == 2'b10 & rand_op == 2'b10) ? lb_w :
                        (a_i ^ b_i); 
                    
    assign y_i_0 =      (sel_share == 2'b01 & rand_op == 2'b00) ? lb_w :
                        (sel_share == 2'b01 & rand_op == 2'b01) ? lc_w :
                        (sel_share == 2'b01 & rand_op == 2'b10) ? la_w :
                        (sel_share == 2'b10 & rand_op == 2'b00) ? la_w :
                        (sel_share == 2'b10 & rand_op == 2'b01) ? lb_w :
                        (sel_share == 2'b10 & rand_op == 2'b10) ? lc_w :
                        (b_i ^ c_i); 
                        
    assign x_i_1 =      (sel_share == 2'b10 & rand_op == 2'b00) ? lb_w :
                        (sel_share == 2'b10 & rand_op == 2'b01) ? lc_w :
                        (sel_share == 2'b10 & rand_op == 2'b10) ? la_w :
                        (a_i ^ c_i); 
                    
    assign y_i_1 =      (sel_share == 2'b10 & rand_op == 2'b00) ? lc_w :
                        (sel_share == 2'b10 & rand_op == 2'b01) ? la_w :
                        (sel_share == 2'b10 & rand_op == 2'b10) ? lb_w :
                        (a_i ^ b_i ^ c_i); 
    ...
    generate
        for (i = 0; i < 1600; i = i + 320) begin
            kecti3_tchi tchi_0  (   .f_o(  f_o_0[319 + i: i]  ),
                                    .x_i(  x_i_0[319 + i: i]  ),
                                    .y_i(  y_i_0[319 + i: i]  )   );
            kecti3_tchi tchi_1  (   .f_o(  f_o_1[319 + i: i]  ),
                                    .x_i(  x_i_1[319 + i: i]  ),
                                    .y_i(  y_i_1[319 + i: i]  )   );
        end
    endgenerate
    ...
    assign r_o =   ({8{r_i[0]}} & 8'h1A) ^ ({8{r_i[1]}} & 8'h34) ^
     ...
     always @(posedge clk) begin
        ...
        case(sel_share)
            2'b00: begin
                        if(rand_op == 2'b00) la_w    <= lin_o_0;
                else    if(rand_op == 2'b10) la_w    <= lin_o_1;
                else                         la_w    <= la_w;
                
                        if(rand_op == 2'b01) lb_w    <= lin_o_0;
                else    if(rand_op == 2'b00) lb_w    <= lin_o_1;
                else                         lb_w    <= lb_w;    
                
                        if(rand_op == 2'b10) lc_w    <= lin_o_0;
                else    if(rand_op == 2'b01) lc_w    <= lin_o_1;
                else                         lc_w    <= lc_w;    
            ...
            2'b01: begin
                        if(rand_op == 2'b01) la_w    <= lin_o_0;
                else                         la_w    <= la_w;
                
                        if(rand_op == 2'b10) lb_w    <= lin_o_0;
                else                         lb_w    <= lb_w;
                
                        if(rand_op == 2'b00) lc_w    <= lin_o_0;
                else                         lc_w    <= lc_w;
 
                        if(rand_op == 2'b00) a_o     <= {   f_o_0[1599:64],
                                                            r_i[6] ^ f_o_0[63], f_o_0[62:32],
                                                            r_i[5] ^ f_o_0[31], f_o_0[30:16],
                                                            r_i[4] ^ f_o_0[15], f_o_0[14: 8],
                                                            r_i[3] ^ f_o_0[ 7], f_o_0[ 6: 4],
                                                            r_i[2] ^ f_o_0[ 3], f_o_0[ 2],
                                                            r_i[1] ^ f_o_0[ 1],
                                                            r_i[0] ^ f_o_0[ 0]    };
                else                         a_o <= a_o;
                
                        if(rand_op == 2'b01) b_o     <= f_o_0;
                else                         b_o     <= b_o;
                
                        if(rand_op == 2'b10) c_o     <= f_o_0;
                else                         c_o     <= c_o;
                sel_share <= 2'b10;
            end
            2'b10: begin
                ...
                lc_w <= lc_w;  
                        ...
                        if(rand_op == 2'b01) a_o     <= {   f_o_0[1599:64],
                                                            r_i[6] ^ f_o_0[63], f_o_0[62:32],
                                                            r_i[5] ^ f_o_0[31], f_o_0[30:16],
                                                            r_i[4] ^ f_o_0[15], f_o_0[14: 8],
                                                            r_i[3] ^ f_o_0[ 7], f_o_0[ 6: 4],
                                                            ...
                                                            r_i[1] ^ f_o_0[ 1],
                                                            r_i[0] ^ f_o_0[ 0]    };
                 else if (rand_op == 2'b10)  a_o     <= {   f_o_1[1599:64],
                                                            r_i[6] ^ f_o_1[63], f_o_1[62:32],
                                                            r_i[5] ^ f_o_1[31], f_o_1[30:16],
                                                            r_i[4] ^ f_o_1[15], f_o_1[14: 8],
                                                            r_i[3] ^ f_o_1[ 7], f_o_1[ 6: 4],
                                                            r_i[2] ^ f_o_1[ 3], f_o_1[ 2],
                                                            r_i[1] ^ f_o_1[ 1],
                                                            r_i[0] ^ f_o_1[ 0]    };
                else                         a_o <= a_o;
                
                        if(rand_op == 2'b10) b_o     <= f_o_0;
                else    if(rand_op == 2'b00) b_o     <= f_o_1;
                else                         b_o     <= b_o;
                
                        if(rand_op == 2'b00) c_o     <= f_o_0;
                else    if(rand_op == 2'b01) c_o     <= f_o_1;
                else                         c_o     <= c_o;
